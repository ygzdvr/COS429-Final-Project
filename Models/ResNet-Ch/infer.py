...
def run_inference(model, image_path, pc_path, transform, save_path):
    ...
    actual_pc = torch.tensor(np.load(pc_path), device=device)  # Convert to tensor on the same device

    # Run inference
    with torch.no_grad():
        pred_pc = model(image).squeeze(0)  # Predicted point cloud as tensor


    mse = torch.mean((pred_pc - actual_pc) ** 2).item()
    print(f"Mean Squared Error (MSE) between predicted and actual point clouds: {mse}")
...
if __name__ == "__main__":
    # Paths
    model_path = "Models/ResNet-Ch/best_model_single_shot.pth"  # Trained model path
    image_path = "Dataset/Images/LowResolution/test/airplane/airplane_0627/1.png"  # Test image path
    pc_path = "Dataset/PointClouds/VeryLowResolution/test/airplane/airplane_0627.npy"  # Corresponding point cloud path
    output_path = "Models/ResNet-Ch/pointcloud_comparison_1.png"  # Output plot path
