...
os.makedirs("Models/ResNet-MSE/model_archive", exist_ok=True)

print("Training loop")
# Save the best model
best_validation_loss = float('inf')
best_train_loss = float('inf')
best_model_path = "Models/ResNet-MSE/best_model_single_shot.pth"
...
def plot_pointclouds(pred_pc, actual_pc, save_path):
    """
    Plots the predicted and actual point clouds and saves the figure.
    """
...
for epoch in range(50):  # Number of epochs
    ...
    epoch_model_path = f"Models/ResNet-MSE/model_archive/model_epoch_{epoch}.pth"
    ...
    plot_pointclouds(train_pred_pc, train_sample_pc.numpy(), f"Models/ResNet-MSE/epoch_archive/train_epoch_{epoch}.png")
    plot_pointclouds(val_pred_pc, val_sample_pc.numpy(), f"Models/ResNet-MSE/epoch_archive/val_epoch_{epoch}.png")
...
plt.ylabel("Loss")
plt.legend()
plt.savefig("Models/ResNet-MSE/training_loss_plot.png")
plt.close()
...
plt.ylabel("Loss")
plt.legend()
plt.savefig("Models/ResNet-MSE/validation_loss_plot.png")
plt.close()
...
plt.ylabel("Loss")
plt.legend()
plt.savefig("Models/ResNet-MSE/training_validation_loss_plot.png")
plt.close()
